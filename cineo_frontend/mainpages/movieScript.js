...
/*
function toggleDescription(summaryElement, toggleButton) {
  const isExpanded = toggleButton.textContent === "Weniger"; // Prüfen, ob der Text aktuell erweitert ist
  const fullText = summaryElement.getAttribute("data-full-text"); // Vollständiger Text
  const maxLength = 350; // Maximale Zeichenanzahl

  if (isExpanded) {
    // Text kürzen
    let shortenedText = fullText.substring(0, maxLength);

    // Suche das letzte Leerzeichen innerhalb des gekürzten Texts
    const lastSpaceIndex = shortenedText.lastIndexOf(' ');
    if (lastSpaceIndex !== -1) {
      shortenedText = fullText.substring(0, lastSpaceIndex) + "...";
    } else {
    ...
    // Klasse entfernen, damit die Höhe zurückgesetzt wird
    ...
    // Klasse hinzufügen, damit sich die Höhe automatisch erweitert
...
function toggleDescription(summaryElement, toggleButton) {
  ...
  if (isExpanded) {
    ...
    // Grid einklappen
    showtimesGrid.style.maxHeight = "0px";
    showtimesGrid.style.opacity = "0";
  } else {
    // Volltext anzeigen
    summaryElement.textContent = fullText;
    toggleButton.textContent = "Weniger";
    ...
    // Grid ausklappen
    showtimesGrid.style.maxHeight = `${showtimesGrid.scrollHeight}px`;
    showtimesGrid.style.opacity = "1";
  }
}
  }
*/
